...
export default function Home() {
  ...
  const [isPro, setIsPro] = useState<boolean>(false); // NEW: Subscription state
    ...
    <div className="min-h-screen relative overflow-hidden text-zinc-100">
      ...
      <header className="sticky top-0 z-50 border-b border-white/5 bg-black/20 backdrop-blur-2xl">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            {!isPro && (
              <button
                onClick={() => setIsPro(true)} // Mocking upgrade
                className="ml-4 px-2 py-0.5 rounded-md bg-gradient-to-r from-amber-400 to-amber-600 text-[10px] font-black text-black uppercase tracking-tighter hover:scale-105 transition-transform"
              >
                Upgrade to Pro
              </button>
            )}
            {isPro && (
              <div className="ml-4 px-2 py-0.5 rounded-md border border-amber-500/30 bg-amber-500/10 text-[10px] font-black text-amber-500 uppercase tracking-tighter">
                Pro Member
              </div>
            )}
          </div>
      ...
      <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
        <BentoGrid>
          ...
          {/* Charts/Analytics - Gated Feature */}
          <BentoItem colSpan={2} className="relative overflow-hidden">
            {!isPro && (
              <div className="absolute inset-0 z-10 bg-black/60 backdrop-blur-sm flex flex-col items-center justify-center p-6 text-center">
                <div className="h-12 w-12 rounded-full bg-amber-500/20 flex items-center justify-center text-amber-500 mb-4">
                  <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" /></svg>
                </div>
                <h4 className="text-lg font-bold mb-1">Advanced Analytics</h4>
                <p className="text-sm text-zinc-400 mb-4 max-w-xs">Unlock detailed patterns and historical trends with Legend Pro.</p>
                <button
                  onClick={() => setIsPro(true)}
                  className="px-6 py-2 rounded-xl bg-amber-500 text-black font-bold text-xs hover:bg-amber-400 transition-all active:scale-95 shadow-lg shadow-amber-500/20"
                >
                  Go Pro - $9.99/mo
                </button>
              </div>
